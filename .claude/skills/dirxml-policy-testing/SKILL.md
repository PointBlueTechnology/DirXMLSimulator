...
| **A GCV value** (incl. DriverSet/Library scope) | `WARNING: GCV(s) referenced but not defined` (or a value resolves empty); or a stage fails to **build** with `GCV 'x' is referenced as ~x~ but is not defined` — the engine substitutes `~name~` into the policy text when it loads it, and an undefined one stops the *driver* from starting, so the harness stops too | The value — set `gcv.<name>=<value>` in case.properties (quickest) or a `gcv.xml` override; or use a **full driver-set source** (project/LDIF/live, or re-export with referenced policies) which carries the DriverSet GCV block a single-driver export may omit. |
